...
export function MeghalayaDashboard({ onBack }: MeghalayaDashboardProps) {
  ...
  const [audit, setAudit] = useState<MeghalayaPredictionResponse | null>(null);
  ...
  useEffect(() => {
    ...
    // Real per-cell prediction, taken from the compact /predict/meghalaya/map
   ...
   * fetched from /predict/meghalaya/grid the first time an operator opens the panel —
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7">
            ...
            <LiveRainfallPanel state="meghalaya" className="mt-4" />

            {/* Predicted risk zones — the persisted Meghalaya model over the AOI grid,
                drawn from the compact /predict/meghalaya/map projection. Rainfall wording
            ...
            <div className="mt-4 space-y-3">
              {predictionState === 'loading' && (
                <div className="flex items-center gap-2 rounded-lg border border-slate-800 bg-slate-900/40 p-3 font-mono text-[11px] text-slate-400">
                  ...
                  Running the Meghalaya model over the AOI grid…
                </div>
              )}
