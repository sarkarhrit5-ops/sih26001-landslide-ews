/**
 * Sikkim validated-pilot console.
 *
 * Every figure on this page is fetched live from the persisted evidence bundle
 * (GET /api/v1/validation/sikkim/evidence) and the real GLC positives
 * (GET /api/v1/validation/sikkim/events). Nothing is fabricated:
 *   - model metrics come from the held-out evaluation the backend serves;
 *   - the land-cover feature is shown honestly as a DERIVED_PROXY;
 *   - IMERG satellite rainfall is shown UNAVAILABLE (Earthdata auth missing);
...
import { SikkimMap } from '../components/sikkim/SikkimMap';
import { toPilotMapCells } from '../components/pilot/pilotMapCells';
import { apiService } from '../services/api';
import type {
  PilotMapResponse,
  SikkimEvidenceResponse,
  SikkimEventsResponse,
  SikkimPredictionResponse,
  ValidationMetricSet,
} from '../services/api';

type LoadState = 'loading' | 'ready' | 'error';
/** The audit disclosure is fetched lazily, so it has an extra pre-request state. */
...
function ModelComparisonTable({
  ...
  comparison: SikkimEvidenceResponse['metrics'] extends null
    ? never
    : NonNullable<SikkimEvidenceResponse['metrics']>['model_comparison'];
}) {
...
/** How each of the 11 model features is sourced (honest per-feature provenance). */
function featureKind(name: string): string {
  if (name === 'land_cover_class') return 'DERIVED_PROXY';
  return 'REAL';
}

function FeatureSchemaPanel({ schema }: { schema: SikkimEvidenceResponse['feature_schema'] }) {
    ...
    <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
      ...
      <div className="grid gap-1.5 sm:grid-cols-2">
        {schema.feature_names.map((f) => {
          ...
          const proxy = featureKind(f) === 'DERIVED_PROXY';
          return (
            <div
              key={f}
              className={cn(
...
function ProvenancePanel({
  ...
  provenance: SikkimEvidenceResponse['provenance'];
}) {
  if (!provenance) {
    return (
      <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5 text-[13px] text-slate-400">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
        ...
        <p className="mt-2 text-[10px] leading-relaxed text-slate-500">
          Mirrors the backend provenance artifact verbatim. IMERG satellite rainfall was not used in
          this training run; the live rainfall layer is separately shown unavailable pending
...
interface SikkimDashboardProps {
  onBack: () => void;
}

export function SikkimDashboard({ onBack }: SikkimDashboardProps) {
  const [evidence, setEvidence] = useState<SikkimEvidenceResponse | null>(null);
  const [events, setEvents] = useState<SikkimEventsResponse | null>(null);
  ...
  const [audit, setAudit] = useState<SikkimPredictionResponse | null>(null);
  ...
  useEffect(() => {
    ...
    // Real per-cell prediction, taken from the compact /predict/sikkim/map
    // projection: one request for the whole grid, no per-cell request, and no
    // /grid fetch on initial load. On refusal (HTTP 503) we keep the backend's
    // own reason and show no zones rather than substituting fabricated risk.
    apiService
      .getPilotMap('sikkim')
      .then((d) => {
        if (!alive) return;
   ...
   * fetched from /predict/sikkim/grid the first time an operator opens the panel —
    ...
    <div className="min-h-screen bg-[#0a0d12] text-slate-100">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <Eyebrow>East Sikkim · Validated pilot</Eyebrow>
              <StatusPill tone="pilot">Validated pilot</StatusPill>
            </div>
            <h1 className="mt-2 text-2xl font-bold tracking-tight text-slate-50 sm:text-3xl">
              Sikkim landslide susceptibility console
            </h1>
        ...
        {evidenceState === 'error' && (
          <div className="mt-5 flex items-start gap-3 rounded-xl border border-amber-500/25 bg-amber-500/[0.05] p-4">
            ...
            <div className="text-[13px] leading-relaxed text-slate-300">
              ...
              <code className="font-mono text-slate-400">/api/v1/validation/sikkim/evidence</code>{' '}
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7">
            ...
            <LiveRainfallPanel state="sikkim" className="mt-4" />

            {/* Predicted risk zones — the persisted model over the AOI grid, drawn
                from the compact /predict/sikkim/map projection. Rainfall wording is
                no longer an unconditional "real IMERG" claim: the banner reads the
                provenance the backend actually reported, so the page never asserts
                an IMERG observation for an Open-Meteo ERA5 fallback series, never
                claims rainfall is missing when it was used, and never claims a
                prediction when the backend refused. */}
            <div className="mt-4 space-y-3">
              {predictionState === 'loading' && (
                <div className="flex items-center gap-2 rounded-lg border border-slate-800 bg-slate-900/40 p-3 font-mono text-[11px] text-slate-400">
                  ...
                  Running the pilot model over the AOI grid…
                </div>
              )}
                  ...
                  {prediction.summary && (
                    <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
                      ...
                      <MiniStat
                        ...
                        suffix="%"
                      />
                      <MiniStat v={prediction.summary.cells_unavailable} l="no terrain" />
                    </div>
                  )}
