/**
 * Arunachal Pradesh validated-pilot console.
 *
 * Every figure on this page is fetched live from the persisted Arunachal evidence
 * bundle (GET /api/v1/validation/arunachal/evidence), the real GLC positives
 * (GET /api/v1/validation/arunachal/events), and a live per-cell prediction
 * (GET /api/v1/predict/arunachal/grid). Nothing is fabricated:
 ...
 *     (the same treatment as the Assam pilot);
 ...
 *
 * Structurally this is the Sikkim console adapted to the Arunachal endpoints, whose
 * JSON shapes are identical. SikkimDashboard and AssamDashboard are left untouched.
 */
...
import type {
  ArunachalEvidenceResponse,
  ArunachalEventsResponse,
  ArunachalPredictionResponse,
  PilotMapResponse,
  ValidationMetricSet,
} from '../services/api';
...
function ModelComparisonTable({
  ...
  comparison: ArunachalEvidenceResponse['metrics'] extends null
    ? never
    : NonNullable<ArunachalEvidenceResponse['metrics']>['model_comparison'];
}) {
...
/**
 * How each of the 11 model features is sourced. The Arunachal pilot's single
 ...
 * Arunachal feature is REAL.
 */
function featureKind(_name: string): string {
  return 'REAL';
}

function FeatureSchemaPanel({ schema }: { schema: ArunachalEvidenceResponse['feature_schema'] }) {
...
function ProvenancePanel({
  ...
  provenance: ArunachalEvidenceResponse['provenance'];
}) {
  if (!provenance) {
    return (
      <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5 text-[13px] text-slate-400">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
        ...
        <p className="mt-2 text-[10px] leading-relaxed text-slate-500">
          Mirrors the backend provenance artifact verbatim. The Arunachal model was trained on
...
interface ArunachalDashboardProps {
  onBack: () => void;
}

export function ArunachalDashboard({ onBack }: ArunachalDashboardProps) {
  const [evidence, setEvidence] = useState<ArunachalEvidenceResponse | null>(null);
  const [events, setEvents] = useState<ArunachalEventsResponse | null>(null);
  ...
  const [audit, setAudit] = useState<ArunachalPredictionResponse | null>(null);
  ...
  useEffect(() => {
    ...
    // Real per-cell prediction, taken from the compact /predict/arunachal/map
    // projection: one request for the whole grid, no per-cell request, and no /grid
    // fetch on initial load. On refusal (HTTP 503) we keep the backend's own reason
    // and show no zones rather than substituting fabricated risk.
    apiService
      .getPilotMap('arunachal')
      .then((d) => {
        if (!alive) return;
        setPrediction(d);
   ...
   * fetched from /predict/arunachal/grid the first time an operator opens the panel —
    ...
    <div className="min-h-screen bg-[#0a0d12] text-slate-100">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <Eyebrow>Arunachal Pradesh · central Subansiri-Siang pilot</Eyebrow>
              {evidenceState === 'ready' && (
            ...
            <h1 className="mt-2 text-2xl font-bold tracking-tight text-slate-50 sm:text-3xl">
              Arunachal Pradesh landslide susceptibility console
            </h1>
        ...
        {evidenceState === 'error' && (
          <div className="mt-5 flex items-start gap-3 rounded-xl border border-amber-500/25 bg-amber-500/[0.05] p-4">
            ...
            <div className="text-[13px] leading-relaxed text-slate-300">
              ...
              <code className="font-mono text-slate-400">/api/v1/validation/arunachal/evidence</code>{' '}
            ...
            <div className="mt-4 space-y-3">
              {predictionState === 'loading' && (
                <div className="flex items-center gap-2 rounded-lg border border-slate-800 bg-slate-900/40 p-3 font-mono text-[11px] text-slate-400">
                  ...
                  Running the Arunachal model over the AOI grid…
                </div>
              )}
