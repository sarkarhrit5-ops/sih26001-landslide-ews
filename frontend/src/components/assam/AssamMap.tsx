 ...
 *
 * This is a sibling of SikkimMap: same rendering contract, and both now consume the
 * compact /predict/<state>/map projection through the same shared pure helpers in
 * ../pilot/pilotMapCells. That payload omits the per-cell bbox, so each rectangle is
 * reconstructed from the grid's own cell_height_deg / cell_width_deg around the cell
 * centre the model was sampled at; when the grid reports no usable cell size, no
 * rectangle is drawn rather than an assumed extent. Only the AOI constant and two
 * labels differ between the pilots, and both come from the pilot registry.
 */
...
interface AssamMapProps {
  ...
  loaded: boolean;
  /** Predicted cells from /predict/assam/map; omitted/empty when no prediction is available. */
  predictedCells?: PilotMapCell[];
}

const PILOT = getPilotConfig('assam');
const AOI = PILOT.aoi;
