/**
 * Arunachal Pradesh pilot map — the exact modelled AOI rectangle, the real NASA GLC
 * landslide positives inside it, and (when available) the model's predicted per-grid-cell
 ...
 *
 * This is a sibling of SikkimMap / AssamMap: same rendering contract, and all four
 * pilots now consume the compact /predict/<state>/map projection through the same
 * shared pure helpers in ../pilot/pilotMapCells. That payload omits the per-cell bbox,
 * so each rectangle is reconstructed from the grid's own cell_height_deg /
 * cell_width_deg around the cell centre the model was sampled at; when the grid
 * reports no usable cell size, no rectangle is drawn rather than an assumed extent.
 * Only the AOI constant and two labels differ, and both come from the pilot registry.
 */
...
interface ArunachalMapProps {
  ...
  loaded: boolean;
  /** Predicted cells from /predict/arunachal/map; omitted/empty when no prediction is available. */
  predictedCells?: PilotMapCell[];
}

const PILOT = getPilotConfig('arunachal');
const AOI = PILOT.aoi;
const AOI_CENTER: [number, number] = [(AOI.minLat + AOI.maxLat) / 2, (AOI.minLon + AOI.maxLon) / 2];

export function ArunachalMap({ events, loaded, predictedCells }: ArunachalMapProps) {
  ...
  useEffect(() => {
    ...
    // The Arunachal AOI is wider in longitude (~2.5°) than Sikkim's, so it frames at a
