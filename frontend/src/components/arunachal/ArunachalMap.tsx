 ...
 *
 * This is a sibling of SikkimMap / AssamMap: same rendering contract, only the AOI
 * constant and two labels differ. SikkimMap and AssamMap are left untouched.
 */
import { useEffect, useRef } from 'react';
import L from 'leaflet';
import type { LandslideEvent, ArunachalPredictionCell } from '../../services/api';
import { ARUNACHAL_PILOT_AOI } from '../../data/nerStates';
...
interface ArunachalMapProps {
  ...
  predictedCells?: ArunachalPredictionCell[];
}
...
export function ArunachalMap({ events, loaded, predictedCells }: ArunachalMapProps) {
  ...
  useEffect(() => {
    ...
    ).bindTooltip('Arunachal Pradesh pilot AOI — modelled extent', { direction: 'top', opacity: 0.95 });
    aoiRect.addTo(layer);
