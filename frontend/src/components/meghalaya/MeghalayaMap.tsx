 ...
 * only the AOI constant and two labels differ. The other pilot maps are left untouched.
 */
import { useEffect, useRef } from 'react';
import L from 'leaflet';
import type { LandslideEvent, MeghalayaPredictionCell } from '../../services/api';
import { MEGHALAYA_PILOT_AOI } from '../../data/nerStates';
...
interface MeghalayaMapProps {
  ...
  predictedCells?: MeghalayaPredictionCell[];
}
...
export function MeghalayaMap({ events, loaded, predictedCells }: MeghalayaMapProps) {
  ...
  useEffect(() => {
    ...
    ).bindTooltip('Meghalaya pilot AOI — modelled extent', { direction: 'top', opacity: 0.95 });
    aoiRect.addTo(layer);
