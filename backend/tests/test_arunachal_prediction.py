"""
Focused offline tests for app.services.arunachal_prediction -- the read-only grid
prediction service that runs the persisted 11-feature ARUNACHAL PRADESH LightGBM with
real IMERG antecedent rainfall.

This mirrors test_assam_prediction.py (same dependency budget: stdlib +
numpy/pandas only; model, terrain sampler, land-cover resolver and rainfall
provider are injected as fakes, so these tests exercise the ASSEMBLY and the
NO-FABRICATION invariants -- not a real prediction, which is host-only). The only
heavy module touched is app.models.ml_pipeline (for the real
calculate_warning_level banding); its top-level lightgbm/shap/sklearn imports are
stubbed here IF absent, exactly as the offline shim harness does, so the REAL
banding function is what runs.

It additionally pins the TWO -- and only two -- ways the Arunachal Pradesh path
differs from Sikkim (identical to the Assam pilot):
...
As with test_assam_prediction, the point is that when an input is unavailable the
service SAYS SO (status UNAVAILABLE / HTTP-503-mapped refusal) instead of inventing
a probability, a coordinate, a land-cover class, or a rainfall value.
"""
...
from app.services import arunachal_prediction as ap
...
def _valid_evidence(model):
    return {
        "status": "VALID",
        "model": model,
        "feature_schema": {"feature_names": list(ap.MODEL_FEATURE_ORDER)},
...
def _run(step=0.25, date="2025-09-19", terrain_sampler=_pattern_terrain_sampler,
    ...
    result = ap.predict_arunachal_grid(
        date,
        step_deg=step,
...
# ---------------------------------------------------------------------------
# Grid geometry (over the canonical ARUNACHAL PRADESH AOI, not Sikkim's)
# ---------------------------------------------------------------------------
def test_grid_centers_are_strictly_inside_the_arunachal_pilot_aoi():
    _bounds, meta, cells = sp.build_grid(step_deg=0.25, state_name=ap.STATE_NAME)
    ...
    for cell in cells:
        assert risk_inputs.point_within_pilot_aoi(
            cell["latitude"], cell["longitude"], state_name=ap.STATE_NAME
        ), cell
        bbox = cell["bbox"]
        assert bbox["min_lat"] < bbox["max_lat"] and bbox["min_lon"] < bbox["max_lon"]
    # This AOI really is the Arunachal Pradesh one, disjoint from Sikkim's DEM extent.
    for cell in cells:
...
def test_state_pilot_area_and_model_contract():
    result, _model = _run()
    assert result["state"] == "Arunachal Pradesh"
    assert result["pilot_area"] == ap.PILOT_AREA == "central Subansiri-Siang belt"
    assert result["target_date"] == "2025-09-19"
    assert result["decision_threshold"] == ap.DECISION_THRESHOLD
    assert result["model"]["feature_order"] == list(ap.MODEL_FEATURE_ORDER)
    ...
    assert list(ap.MODEL_FEATURE_ORDER) == list(sp.MODEL_FEATURE_ORDER)


# ---------------------------------------------------------------------------
# THE Arunachal difference #1: land cover scored as a CATEGORICAL over 1..6
# ---------------------------------------------------------------------------
def test_land_cover_scored_as_categorical_over_fixed_worldcover_vocabulary():
    result, model = _run()

    frame = model.frames[0]
    assert list(frame.columns) == list(ap.MODEL_FEATURE_ORDER)
...
# ---------------------------------------------------------------------------
# THE Arunachal difference #2: land cover is REAL WorldCover, not an elevation proxy
# ---------------------------------------------------------------------------
...
def test_score_cells_categorical_builds_fixed_vocabulary_and_picks_positive_class():
    ...
    out1 = ap._score_cells_categorical(m1, rows)
    assert out1 == [0.8, 0.8]
    ...
    out2 = ap._score_cells_categorical(m2, rows)
    assert out2 == [0.8, 0.8]


# ---------------------------------------------------------------------------
# _arunachal_land_cover: real groups mapped, nodata refused (never filled)
# ---------------------------------------------------------------------------
def test_arunachal_land_cover_maps_real_groups_and_refuses_nodata(tmp_path):
    raw_dir = tmp_path / "raw"
    raw_dir.mkdir()
    (raw_dir / ap.ARUNACHAL_LANDCOVER_FILENAME).write_bytes(b"not-a-real-tif-but-nonzero")
    ...
    centers = [(27.0, 93.0), (27.1, 93.1), (27.2, 93.2)]
    res = ap._arunachal_land_cover(centers, data_dir=str(tmp_path), reader=_reader)
...
def test_arunachal_land_cover_refuses_when_raster_missing(tmp_path):
    # No land-cover raster on disk -> a SYSTEMIC refusal (mapped to HTTP 503), not a
    # per-cell None.
    with pytest.raises(ap.PredictionUnavailable):
        ap._arunachal_land_cover([(27.0, 93.0)], data_dir=str(tmp_path))


# ---------------------------------------------------------------------------
# Path helpers point at the ARUNACHAL rasters (DEM+landcover in raw/, derivatives in processed/)
# ---------------------------------------------------------------------------
def test_path_helpers_point_at_arunachal_rasters():
    paths = ap.arunachal_terrain_raster_paths(data_dir="/tmp/somewhere")
    assert set(paths) == {"elevation", "slope", "aspect", "roughness", "tpi"}
    assert paths["elevation"].endswith(os.path.join("raw", "arunachal_pilot_dem.tif"))
    for name in ("slope", "aspect", "roughness", "tpi"):
        assert paths[name].endswith(
            os.path.join("processed", "arunachal_pilot_%s.tif" % name)
        )
    lc = ap.arunachal_landcover_raster_path(data_dir="/tmp/somewhere")
    assert lc.endswith(os.path.join("raw", "arunachal_pilot_landcover.tif"))
...
def test_refuses_on_feature_order_mismatch():
    ...
    evidence["feature_schema"]["feature_names"] = list(ap.MODEL_FEATURE_ORDER)[::-1]
    with pytest.raises(ap.PredictionUnavailable) as exc:
        ap.predict_arunachal_grid(
            "2025-09-19", step_deg=0.25,
    ...
    assert exc.value.details["expected"] == list(ap.MODEL_FEATURE_ORDER)
