"""
Focused offline tests for app.services.assam_prediction -- the read-only grid
prediction service that runs the persisted 11-feature ASSAM LightGBM with real
IMERG antecedent rainfall.

This mirrors test_sikkim_prediction.py (same dependency budget: stdlib +
...
It additionally pins the TWO -- and only two -- ways the Assam path differs from
Sikkim:
...
As with test_sikkim_prediction, the point is that when an input is unavailable the
...
# ---------------------------------------------------------------------------
# Grid geometry (over the canonical ASSAM AOI, not Sikkim's)
# ---------------------------------------------------------------------------
def test_grid_centers_are_strictly_inside_the_assam_pilot_aoi():
    ...
    # This AOI really is the Assam one, disjoint from Sikkim's DEM extent.
    for cell in cells:
...
def test_state_pilot_area_and_model_contract():
    ...
    assert result["pilot_area"] == ap.PILOT_AREA == "Guwahati-Kamrup + western Karbi Anglong"
...
# ---------------------------------------------------------------------------
# THE Assam difference #1: land cover scored as a CATEGORICAL over 1..6
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# THE Assam difference #2: land cover is REAL WorldCover, not an elevation proxy
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _assam_land_cover: real groups mapped, nodata refused (never filled)
# ---------------------------------------------------------------------------
def test_assam_land_cover_maps_real_groups_and_refuses_nodata(tmp_path):
    raw_dir = tmp_path / "raw"
    raw_dir.mkdir()
    (raw_dir / ap.ASSAM_LANDCOVER_FILENAME).write_bytes(b"not-a-real-tif-but-nonzero")
    ...
    centers = [(26.0, 92.0), (26.1, 92.1), (26.2, 92.2)]
    res = ap._assam_land_cover(centers, data_dir=str(tmp_path), reader=_reader)
...
def test_assam_land_cover_refuses_when_raster_missing(tmp_path):
    # No land-cover raster on disk -> a SYSTEMIC refusal (mapped to HTTP 503), not a
    # per-cell None.
    with pytest.raises(ap.PredictionUnavailable):
        ap._assam_land_cover([(26.0, 92.0)], data_dir=str(tmp_path))


# ---------------------------------------------------------------------------
# Path helpers point at the ASSAM rasters (DEM+landcover in raw/, derivatives in processed/)
# ---------------------------------------------------------------------------
def test_path_helpers_point_at_assam_rasters():
    paths = ap.assam_terrain_raster_paths(data_dir="/tmp/somewhere")
    assert set(paths) == {"elevation", "slope", "aspect", "roughness", "tpi"}
    assert paths["elevation"].endswith(os.path.join("raw", "assam_pilot_dem.tif"))
    for name in ("slope", "aspect", "roughness", "tpi"):
        assert paths[name].endswith(
            os.path.join("processed", "assam_pilot_%s.tif" % name)
        )
    lc = ap.assam_landcover_raster_path(data_dir="/tmp/somewhere")
    assert lc.endswith(os.path.join("raw", "assam_pilot_landcover.tif"))
