"""
Offline tests for scripts/train_arunachal_model.py -- the Arunachal Pradesh pilot
trainer that loads the completed 200x11 matrix and reproduces the Sikkim/Assam
...
What they protect:
  * the 11-feature schema and order (5 terrain + land_cover_class + 5 rainfall) match
    the Sikkim/Assam trainer, with land_cover_class at index 5 and the temporal cutoff 2014;
  ...
  * the Arunachal label overrides are truthful WorldCover wording and relabelling never
...
import train_arunachal_model as t
...
def test_temporal_cutoff_and_state_labels():
    # Reuse Sikkim's cutoff verbatim (verified non-degenerate for Arunachal:
    # 20 positives train <=2014 / 30 test >=2015).
    assert t.TEMPORAL_CUTOFF_YEAR == 2014
    assert t.STATE_NAME == "Arunachal Pradesh"
...
def _matrix_with_missing():
    ...
    df["latitude"] = np.linspace(26.6, 27.9, 5).astype("float64")
    df["longitude"] = np.linspace(92.1, 94.4, 5).astype("float64")
...
# --------------------------------------------------------------------------- #
# Arunachal label overrides (truthful WorldCover wording; relabel is value-safe)
# --------------------------------------------------------------------------- #
def test_land_cover_meaning_is_worldcover_not_proxy():
    m = t.arunachal_land_cover_meaning()
    low = m.lower()
...
def test_arunachal_feature_meanings_overrides_only_land_cover():
    meanings = t.arunachal_feature_meanings()
    assert list(meanings.keys()) == ["land_cover_class"]


def test_relabel_for_arunachal_changes_only_labels():
    metrics_like = {
        "state": "Sikkim",
    ...
    out = t.relabel_for_arunachal(dict(metrics_like))
    assert out["state"] == "Arunachal Pradesh"
    ...
    out2 = t.relabel_for_arunachal(dict(schema_like))
    assert out2["state"] == "Arunachal Pradesh"
...
def test_evaluate_baselines_only_lgbm_is_categorical_and_consistent():
    ...
    with _inject_fake_sklearn():
        res = t.evaluate_baselines_arunachal(ml, X_train, X_test, y_train, y_test, cats)
