"""
Offline tests for scripts/train_assam_model.py -- the Assam pilot trainer that
loads the completed 236x11 matrix and reproduces the Sikkim methodology, changing
exactly one thing: land_cover_class is fed to the LightGBM primary model as a
NOMINAL categorical (real ESA WorldCover), not a numeric ordinal proxy.
...
What they protect:
  * the 11-feature schema and order (5 terrain + land_cover_class + 5 rainfall) match
    the Sikkim trainer, with land_cover_class at index 5 and the temporal cutoff 2014;
  ...
  * the Assam label overrides are truthful WorldCover wording and relabelling never
...
def test_temporal_cutoff_and_state_labels():
    # Reuse Sikkim's cutoff verbatim (verified non-degenerate for Assam).
    assert t.TEMPORAL_CUTOFF_YEAR == 2014
    assert t.STATE_NAME == "Assam"
...
def _matrix_with_missing():
    ...
    df["latitude"] = np.linspace(25.7, 26.5, 5).astype("float64")
    df["longitude"] = np.linspace(91.4, 93.6, 5).astype("float64")
    df["event_date"] = pd.to_datetime(
        ["2011-06-01", "2013-07-02", "2014-08-03", "2016-09-04", "2017-10-05"]
    )
...
# --------------------------------------------------------------------------- #
# Assam label overrides (truthful WorldCover wording; relabel is value-safe)
# --------------------------------------------------------------------------- #
def test_land_cover_meaning_is_worldcover_not_proxy():
    m = t.assam_land_cover_meaning()
    low = m.lower()
...
def test_assam_feature_meanings_overrides_only_land_cover():
    meanings = t.assam_feature_meanings()
    assert list(meanings.keys()) == ["land_cover_class"]


def test_relabel_for_assam_changes_only_labels():
    metrics_like = {
        "state": "Sikkim",
    ...
    out = t.relabel_for_assam(dict(metrics_like))
    assert out["state"] == "Assam"
    ...
    out2 = t.relabel_for_assam(dict(schema_like))
    assert out2["state"] == "Assam"
...
def test_evaluate_baselines_only_lgbm_is_categorical_and_consistent():
    ...
    with _inject_fake_sklearn():
        res = t.evaluate_baselines_assam(ml, X_train, X_test, y_train, y_test, cats)
