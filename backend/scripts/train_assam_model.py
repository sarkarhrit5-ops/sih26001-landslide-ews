#!/usr/bin/env python3
"""
Train the Assam pilot landslide model from the completed 236x11 training matrix.

This is the Assam analogue of ``scripts/train_real_models.py`` sections 4-7
(validate -> decide -> persist). It deliberately REUSES the Sikkim methodology
verbatim through ``app.models.ml_pipeline`` and ``app.services.model_artifacts``
and changes exactly one thing:
    ...
    * ``land_cover_class`` is fed to the LightGBM PRIMARY model as a pandas
      ...
      the Assam feature is a REAL, NOMINAL ESA WorldCover class grouping
      (forest / shrub / cropland / built-up / bare / water) that has no natural
      order -- unlike Sikkim's elevation-binned proxy, which is an ordered
      1 < 2 < 3 encoding and is therefore left numeric there.

Everything else is identical to Sikkim: the 11-feature order, the latitude-median
spatial holdout, the event-year temporal holdout (cutoff 2014), the buffered
negative sampling already baked into the matrix, the antecedent-only rainfall
already baked into the matrix, the reproducibility gate, the Option A/C decision
rule and the four persisted artifacts.
...
Hard integrity rules (shared with the whole project):
    ...
    * It does NOT rebuild features and it does NOT touch the network: it loads
      the matrix that ``scripts/build_assam_training_matrix.py`` already wrote.
    * It does NOT modify Sikkim, IMERG (``weather_ingestion.py``), the frontend,
      or any shared service. State/pilot labels that ``model_artifacts`` hardcodes
      to "Sikkim" are corrected on the returned dicts (relabelling only -- no
      metric, feature or provenance value is altered).
...
# Same cutoff Sikkim uses. Verified non-degenerate for Assam (positives:
# 28 train (<=2014) / 31 test (>=2015)).
...
# ---------------------------------------------------------------------------
def assam_land_cover_meaning() -> str:
    """Human-readable meaning string for the Assam land_cover_class feature.
...
def assam_feature_meanings() -> dict:
    ...
    """
    return {wc.LANDCOVER_FEATURE_NAME: assam_land_cover_meaning()}


def relabel_for_assam(doc):
    ...
    they predate the Assam pilot and take no state argument. This rewrites ONLY
    those label fields to the real Assam run. It never touches validation
    metrics, feature names, dtypes, meanings, sample counts or any other value,
    and the persistence gate does not assert on the state string.
    """
...
def train_primary_categorical(ml, X_train, X_test, y_train, y_test, categories):
    """Assam analogue of ml_pipeline.train_primary_model.
...
def evaluate_baselines_assam(ml, X_train, X_test, y_train, y_test, categories):
    """Assam analogue of ml_pipeline.train_and_evaluate_baselines.
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            "Train the Assam pilot landslide model from the completed "
            "assam_pilot_training_matrix.parquet, mirroring the Sikkim "
            "methodology (WorldCover categorical, real rainfall only, leakage "
            "controls). Does not modify Sikkim or IMERG."
        )
    )
    parser.add_argument(
        "--matrix",
        default=None,
        help="Path to assam_pilot_training_matrix.parquet (default: backend/data/processed/).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    print("==========================================================")
    print("     ASSAM PILOT MODEL TRAINING (Sikkim methodology)      ")
    print("==========================================================")
    ...
    if not os.path.exists(matrix_path):
        raise SystemExit(
            "Training matrix not found: %s\nRun scripts/build_assam_training_matrix.py "
    ...
    # --- 5. Leakage controls (same wording as Sikkim, Assam counts) ---
    leakage_checks = {
        "spatial_leakage": (
    ...
    try:
        ...
        provenance_doc = model_artifacts.build_provenance_document(
            ...
            random_seed=42,
            glc_source=(
                "NASA Global Landslide Catalog Export (assam_events.json snapshot), "
                "AOI-filtered"
            ),
            ...
            dem_source=(
                "Copernicus GLO-30 DEM (30 m), Assam pilot tiles, merged and clipped "
                "to the canonical pilot AOI"
            ),
            ...
            extra={
                ...
                "imerg_note": (
                    ...
                    "archive, exactly as in Sikkim training."
                ),
                "matrix_schema_reference": schema_path,
                "aoi_source": (
                    "app.core.config_states.ASSAM_PILOT_AOI -- the single canonical "
                    "Assam pilot AOI used for tile selection, DEM crop, GLC filter "
