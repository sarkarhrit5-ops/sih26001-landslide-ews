"""
Build the MEGHALAYA pilot TRAINING MATRIX (features only -- this script NEVER trains
a model). It is the Meghalaya analogue of scripts/build_arunachal_training_matrix.py
(which is itself the Arunachal analogue of scripts/build_assam_training_matrix.py, in
turn the Assam analogue of the feature-building portion of the Sikkim trainer
scripts/train_real_models.py), with exactly the SAME deliberate substitution the
Assam and Arunachal pilots made: the degenerate elevation land_cover_class proxy is
replaced by the REAL, categorical ESA WorldCover land cover produced by
scripts/prepare_meghalaya_landcover.py.

FEATURE SCHEMA (11 model features, identical layout to the Sikkim/Assam/Arunachal trainers):
    5 terrain (float32) : elevation, slope, aspect, roughness, tpi
        -> sampled from the on-disk Meghalaya rasters (meghalaya_pilot_dem.tif +
           meghalaya_pilot_{slope,aspect,roughness,tpi}.tif) with the SAME
    ...
    1 land cover (int32, CATEGORICAL) : land_cover_class
        -> app.services.worldcover.assign_assam_land_cover_from_raster (grouped,
           nominal, NEVER ordinal). The function name is ASSAM_* for historical
           reasons only -- it is fully state-agnostic (it samples whatever raster
           path it is given, using the general 6-group nominal collapse defined once
           in app.services.worldcover), so it is reused here VERBATIM against the
           Meghalaya raster, exactly as the grouping constants are. Declared
           categorical via worldcover.landcover_categorical_feature() so a future
           trainer passes it to LightGBM's categorical_feature=. This REPLACES
           assign_land_cover_proxy.
    5 rainfall (float32) : rain_1d, rain_3d, rain_7d, antecedent_rain_14d,
        ...
        -> the SAME scripts/train_real_models.fetch_historical_rainfall_series
           ...
           This is the identical rainfall source Sikkim/Assam/Arunachal TRAINING
           already use; NASA IMERG (app.services.weather_ingestion) is the
           SERVING/real-time path and is NOT a training feature -- it is left
           completely untouched. Requests use LOW concurrency + exponential backoff on
           HTTP 429 ("Too many concurrent requests") via fetch_rainfall_with_backoff
           below; that only spaces out identical retries and NEVER alters the fetched
           values, the antecedent dates, or the source, and never substitutes/fills.

SAMPLES: all real GLC positives from data/models/meghalaya_events.json (34 events,
target=1) plus spatially-buffered negatives from
app.models.ml_pipeline.generate_spatial_negative_samples (target=0, 3:1 -> 102,
>= 0.05 deg from any positive, seed 42), drawn from exactly the canonical Meghalaya
AOI rectangle (== the DEM/land-cover extent) -> 136 rows total. No sample is dropped
by this builder: every positive and every negative is kept.
...
DATA-INTEGRITY CONTRACT (nothing is fabricated or filled):
    ...
    * Rainfall: on ANY failure to obtain a complete real antecedent series for ANY
      ...
      is written (mirrors the Sikkim/Assam/Arunachal trainers). Missing rainfall never
      becomes 0 mm.
...
WHY THE meghalaya_pilot_ NAMES: same collision rationale as the DEM/terrain/landcover
drivers -- the sweep (app.services.state_validation) owns the "meghalaya_" prefix, so
all pilot products use "meghalaya_pilot_" to stay separate. The events snapshot it
consumes is the canonical meghalaya_events.json (single-word state -> underscore form
already matches the serving resolver; no space workaround, unlike Arunachal).
...
Assam nor Arunachal pipeline, IMERG, nor the frontend.

Usage (from backend/ on the host):
    python scripts/build_meghalaya_training_matrix.py
    python scripts/build_meghalaya_training_matrix.py --max-workers 8
"""
import argparse
...
SCHEMA_FILENAME = "meghalaya_pilot_training_matrix_schema.json"

# Feature layout -- identical order/semantics to the Sikkim/Assam/Arunachal trainers'
# static_features + rainfall block, so a Meghalaya trainer can reuse the same code.
...
# archive-api.open-meteo.com request per sample. Fetching all 136 samples with high
...
RAINFALL_DEFAULT_MAX_WORKERS = 2      # low enough to avoid concurrent-429s
...
def load_positive_events(models_dir):
    """
    Load the real Meghalaya GLC positives from the committed events snapshot into a
    ...
    if not events:
        raise SystemExit("Meghalaya events snapshot at %s contains zero events." % path)
    ...
    if n_bad_date:
        ...
        raise SystemExit(
            "%d Meghalaya positive event(s) have an unparseable event_date; refusing to "
...
def terrain_raster_map(raw_dir, proc_dir):
    ...
    if missing:
        raise SystemExit(
            "Missing terrain raster(s) for %s: %s. Run scripts/prepare_meghalaya_terrain.py first."
...
def main():
    parser = argparse.ArgumentParser(
        description="Build the Meghalaya pilot training feature matrix (no model training)."
    )
    parser.add_argument(
    ...
    # __main__) and defines no state-specific behaviour of its own.
    ...
    # 0. Canonical AOI (fail fast if the pilot AOI is not inside Meghalaya's admin bbox).
    ...
    if not os.path.exists(landcover_path):
        raise SystemExit(
            "WorldCover raster not found at %s. Run scripts/prepare_meghalaya_landcover.py first."
            % landcover_path
        )
    raster_map = terrain_raster_map(raw_dir, proc_dir)

    print("=" * 72)
    print(" MEGHALAYA PILOT TRAINING MATRIX (features only; NO training)")
    ...
    #    assign_assam_land_cover_from_raster is state-agnostic (see module docstring):
    #    it samples the given Meghalaya raster with the shared nominal grouping.
    ...
    try:
        ...
        # Mirror the Sikkim/Assam/Arunachal trainers: do NOT fabricate rainfall and do
        # NOT persist a matrix built on incomplete rainfall. No artifact is written.
        raise trm.RainfallUnavailableError(
            "ABORTING: real Open-Meteo ERA5 antecedent rainfall is unavailable for one "
            "or more Meghalaya samples, so a scientifically valid training matrix cannot "
            "be built. No zero/synthetic rainfall is substituted and no matrix artifact "
            "is written. First failure: %s" % e
        ) from e
    ...
    document = {
        ...
        "sources": {
            ...
            "land_cover": {
                ...
                "assigner": "app.services.worldcover.assign_assam_land_cover_from_raster (state-agnostic; reused verbatim)",
        ...
        "negative_sampling": {
            ...
            "seed": 42,
            "domain": "canonical Meghalaya AOI rectangle (== DEM/land-cover extent)",
        },
        "integrity": {
