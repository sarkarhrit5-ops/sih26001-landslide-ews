"""
Prepare the REAL ESA WorldCover 2021 v200 land-cover raster for the canonical Assam
pilot AOI, and report its class distribution. This is the Assam land-cover analogue
of scripts/prepare_assam_terrain.py: it produces the data product that lets the
Assam feature-preparation replace the degenerate elevation land_cover_class proxy
with real satellite-derived land cover, WITHOUT touching the Sikkim pilot, the
state-comparison sweep, IMERG or the frontend.

WHAT IT PRODUCES (under backend/data/, none committed):
    data/raw/assam_pilot_landcover.tif                  -- 2-tile mosaic, cropped to AOI
    data/models/assam_pilot_landcover_classdist.json    -- machine-readable class
                                                           distribution + grouping used

WHY THIS NAME (do not "simplify" to assam_landcover.tif):
    Same reason as assam_pilot_dem.tif -- the bare "assam_" prefix is owned by the
    state-comparison sweep (app.services.state_validation). "assam_pilot_" keeps the
    pilot product cleanly separate from the sweep and from Sikkim. See
    scripts/prepare_assam_terrain.py's docstring for the full rationale.

CATEGORICAL + NODATA CONTRACT (enforced via app.services.worldcover):
    * WorldCover's 11 codes are NOMINAL; they are collapsed into a small set of
      nominal GROUP codes with an explicit lookup (never a threshold), and the model
      must treat land_cover_class as categorical.
...
app.core.config_states.get_pilot_aoi_bounds("Assam") (single source of truth); the
tiles are DERIVED from that AOI by app.services.worldcover.worldcover_tiles_for_bbox
(so the download/crop cannot cover a different area than the positives were filtered
to); the grouping, nodata handling and tile URLs all live in app.services.worldcover
(single definition site) rather than being restated here.
...
Usage (from the backend/ directory, on the host):
    python scripts/prepare_assam_landcover.py            # build if missing
    python scripts/prepare_assam_landcover.py --force    # rebuild even if present
"""
import argparse
...
CLASSDIST_FILENAME = "assam_pilot_landcover_classdist.json"
...
def build_assam_landcover(raw_dir, tile_cache_dir, force=False):
    """
    Download the ESA WorldCover tiles the Assam pilot AOI needs and mosaic+crop them
    to exactly the AOI, writing data/raw/assam_pilot_landcover.tif.
    ...
    print("ESA WorldCover %s %d tiles required by the canonical Assam AOI: %s"
    ...
    if not tile_paths:
        raise RuntimeError("No WorldCover tiles resolved for the Assam pilot AOI.")
    ...
    print("Wrote Assam pilot land cover: %s" % landcover_path)
    return landcover_path, aoi, tiles
...
def aoi_class_histogram(landcover_path, chunk_rows=1024):
    ...
    stay within the 8 GB target (a full 10 m Assam-AOI read would be ~300M pixels).
    Returns {raw_code: pixel_count} including the nodata code 0.
    """
    import numpy as np
    import rasterio
...
def load_event_points(models_dir):
    """
    Load the 59 Assam pilot positives' (lat, lon) from the committed events snapshot.
    Returns (lats, lons). Raises if the snapshot is missing so we never silently
    report a distribution over zero events.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare the real ESA WorldCover land cover for the Assam pilot AOI and report its class distribution."
    )
    parser.add_argument(
        "--force", action="store_true",
        help="Rebuild the land-cover mosaic even if data/raw/assam_pilot_landcover.tif exists.",
    )
    parser.add_argument(
    ...
    # Fail fast if the pilot AOI is not inside Assam's administrative box.
    ...
    print("=" * 70)
    print(" ASSAM PILOT LAND COVER (ESA WorldCover %s %d)" % (wc.WORLDCOVER_VERSION, wc.WORLDCOVER_YEAR))
    ...
    landcover_path, aoi, tiles = build_assam_landcover(raw_dir, tile_cache_dir, force=args.force)
    ...
    if not coverage["fully_covers_canonical_aoi"]:
        print("WARNING: the land-cover raster does NOT fully cover the canonical Assam "
              "AOI. Samples outside coverage will be UNAVAILABLE (nodata) and must not "
              "be reported as covered.")
    _print_ram("after land-cover mosaic")

    # 2. Class distribution -- AOI-wide and at the 59 event points.
    ...
    print("\nGrouped land cover at the %d Assam event points:" % ev_total)
    for code in wc.ASSAM_LANDCOVER_GROUP_CODES:
    ...
    document = {
        ...
        "grouping": {
            "note": (
                "WorldCover codes are NOMINAL; grouped into a small set for the "
                "59-event dataset. land_cover_class must be treated as CATEGORICAL."
            ),
