...
Arunachal Pradesh pilot AOI, and report its class distribution. This is the
Arunachal analogue of scripts/prepare_assam_landcover.py: it produces the data
product that lets the Arunachal feature-preparation use REAL satellite-derived land
cover (a categorical feature) in place of the Sikkim elevation land_cover_class
proxy, WITHOUT touching the Sikkim or Assam pilots, the state-comparison sweep,
IMERG or the frontend.

WHAT IT PRODUCES (under backend/data/, none committed):
    data/raw/arunachal_pilot_landcover.tif                  -- 4-tile mosaic, cropped to AOI
    data/models/arunachal_pilot_landcover_classdist.json    -- machine-readable class
                                                               distribution + grouping used

WHY THIS NAME (do not "simplify" to arunachal_landcover.tif / arunachal_pradesh_*):
    Same reason as arunachal_pilot_dem.tif -- the sweep (app.services.state_validation)
    owns the "arunachal_pradesh_" prefix (clean_state_name = state_name.lower().
    replace(' ', '_')). "arunachal_pilot_" keeps the pilot product cleanly separate
    from the sweep and from the other pilots. See scripts/prepare_arunachal_terrain.py
...
REUSE OF THE WORLDCOVER SERVICE (app.services.worldcover) -- NOT MODIFIED:
    ...
    SINGLE definition site and is NOT edited by this driver, so the Assam pilot that
    also imports it is byte-for-byte unaffected.
    ...
    100->shrub/grass/herbaceous. It is reused verbatim for Arunachal so the two pilots'
    land-cover grouping cannot drift. Note snow (70) maps to group 5, whose label is
    "bare / sparse / snow" -- correct for Arunachal's higher-elevation Eastern Himalaya,
    where snow is more plausible than in Assam; the class report surfaces how much
    actually appears rather than assuming.
...
app.core.config_states.get_pilot_aoi_bounds("Arunachal Pradesh") (single source of
truth); the tiles are DERIVED from that AOI by
app.services.worldcover.worldcover_tiles_for_bbox (so the download/crop cannot cover
a different area than the positives were filtered to) and resolve to the four 3-deg
tiles N24E090, N24E093, N27E090, N27E093. Two of those (N24E090, N24E093) are the
same tiles the Assam pilot uses, so a shared data/raw/worldcover tile cache is reused
rather than re-downloaded.

RAM NOTE (8 GB target): the Arunachal AOI is ~1.56x the Assam pilot's, so a full 10 m
read is several hundred million pixels. The whole-AOI histogram is therefore computed
in row strips (--chunk-rows, default 1024) and _print_ram() reports RSS at each stage.
...
Usage (from the backend/ directory, on the host):
    python scripts/prepare_arunachal_landcover.py            # build if missing
    python scripts/prepare_arunachal_landcover.py --force    # rebuild even if present
"""
import argparse
...
CLASSDIST_FILENAME = "arunachal_pilot_landcover_classdist.json"
# Canonical UNDERSCORE events snapshot (see build_arunachal_events_snapshot.py). Read
# by this exact name, the same way the Assam driver reads assam_events.json.
...
def build_arunachal_landcover(raw_dir, tile_cache_dir, force=False):
    """
    Download the ESA WorldCover tiles the Arunachal pilot AOI needs and mosaic+crop
    them to exactly the AOI, writing data/raw/arunachal_pilot_landcover.tif.
    ...
    print("ESA WorldCover %s %d tiles required by the canonical Arunachal AOI: %s"
    ...
    if not tile_paths:
        raise RuntimeError("No WorldCover tiles resolved for the Arunachal pilot AOI.")
    ...
    print("Wrote Arunachal pilot land cover: %s" % landcover_path)
    return landcover_path, aoi, tiles
...
def aoi_class_histogram(landcover_path, chunk_rows=1024):
    ...
    stay within the 8 GB target (a full 10 m Arunachal-AOI read is several hundred
    million pixels). Returns {raw_code: pixel_count} including the nodata code 0.
    """
    import numpy as np
    import rasterio
...
def load_event_points(models_dir):
    """
    Load the 50 Arunachal pilot positives' (lat, lon) from the committed events
    snapshot. Raises if the snapshot is missing so we never silently report a
    distribution over zero events.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare the real ESA WorldCover land cover for the Arunachal pilot AOI and report its class distribution."
    )
    parser.add_argument(
        "--force", action="store_true",
        help="Rebuild the land-cover mosaic even if data/raw/arunachal_pilot_landcover.tif exists.",
    )
    parser.add_argument(
    ...
    # Fail fast if the pilot AOI is not inside Arunachal's administrative box.
    ...
    print("=" * 70)
    print(" ARUNACHAL PILOT LAND COVER (ESA WorldCover %s %d)" % (wc.WORLDCOVER_VERSION, wc.WORLDCOVER_YEAR))
    ...
    landcover_path, aoi, tiles = build_arunachal_landcover(raw_dir, tile_cache_dir, force=args.force)
    ...
    if not coverage["fully_covers_canonical_aoi"]:
        print("WARNING: the land-cover raster does NOT fully cover the canonical "
              "Arunachal AOI. Samples outside coverage will be UNAVAILABLE (nodata) and "
              "must not be reported as covered.")
    _print_ram("after land-cover mosaic")

    # 2. Class distribution -- AOI-wide and at the 50 event points.
    ...
    print("\nGrouped land cover at the %d Arunachal event points:" % ev_total)
    for code in wc.ASSAM_LANDCOVER_GROUP_CODES:
    ...
    document = {
        ...
        "grouping": {
            "note": (
                ...
                "there for historical reasons and reused verbatim for Arunachal). "
