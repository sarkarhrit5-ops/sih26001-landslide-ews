...
canonical Assam pilot AOI. This is the Assam analogue of the DEM/terrain stage of
scripts/train_real_models.py, extracted into its own driver so the Assam data can
be prepared WITHOUT touching the Sikkim pilot, the state-comparison sweep, IMERG
authentication or the frontend.

WHAT IT PRODUCES (all under backend/data/, none committed):
    data/raw/assam_pilot_dem.tif                 -- 6-tile mosaic, cropped to the AOI
    data/processed/assam_pilot_slope.tif
    data/processed/assam_pilot_aspect.tif
    data/processed/assam_pilot_roughness.tif
    data/processed/assam_pilot_tpi.tif

WHY THESE NAMES (do not "simplify" to assam_dem.tif / assam_slope.tif):
    The Sikkim PILOT deliberately uses region-specific names -- east_sikkim_dem.tif
    and the "real_" terrain prefix -- which are DISTINCT from the state-comparison
    sweep's sikkim_dem.tif / "sikkim_" prefix (app.services.state_validation:
    acquire_state_dem writes "{state}_dem.tif", and process_state runs
    process_dem_in_chunks(state_prefix="assam") -> "assam_*.tif"). Reusing the bare
    "assam_" names here would collide with that sweep track: acquire_state_dem's
    size-guard would then serve this pilot-AOI DEM in place of the sweep's
    admin-bbox DEM, silently changing the 8-state sweep. The "assam_pilot_" names
    keep the pilot cleanly separate, exactly as "east_sikkim_dem" / "real_" do for
    Sikkim, and clash with neither the Sikkim pilot nor the Assam sweep.

REUSE, NOT REINVENTION: every AOI-dependent step reads the AOI from
app.core.config_states.get_pilot_aoi_bounds("Assam") (the single source of truth),
selects Copernicus tiles with the same app.services.state_validation
...
Usage (from the backend/ directory, on the host):
    python scripts/prepare_assam_terrain.py            # build if missing
    python scripts/prepare_assam_terrain.py --force    # rebuild even if present
"""
import argparse
import os
import sys
...
# Pilot-specific output names (see module docstring for why they are NOT "assam_").
DEM_FILENAME = "assam_pilot_dem.tif"
TERRAIN_STATE_PREFIX = "assam_pilot"  # process_dem_in_chunks appends "_" -> assam_pilot_*.tif
...
def build_assam_dem(raw_dir, tile_cache_dir, force=False):
    """
    Download the Copernicus GLO-30 tiles the Assam pilot AOI needs (UNCAPPED) and
    mosaic+crop them to exactly the AOI, writing data/raw/assam_pilot_dem.tif.
    ...
    print("Copernicus GLO-30 tiles required by the canonical Assam AOI: %s" % tiles)
    ...
    if not tile_paths:
        raise RuntimeError("No Copernicus tiles resolved for the Assam pilot AOI.")
    ...
    print("Wrote Assam pilot DEM: %s" % dem_path)
    return dem_path, aoi, tiles
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare the real Copernicus DEM + terrain derivatives for the Assam pilot AOI."
    )
    parser.add_argument(
        "--force", action="store_true",
        help="Rebuild the DEM mosaic even if data/raw/assam_pilot_dem.tif already exists.",
    )
    parser.add_argument(
    ...
    # Fail fast if the pilot AOI is not inside Assam's administrative box; in that
    ...
    print("=" * 70)
    print(" ASSAM PILOT DEM + TERRAIN PREPARATION")
    ...
    dem_path, aoi, tiles = build_assam_dem(raw_dir, tile_cache_dir, force=args.force)
    ...
    if not coverage["fully_covers_canonical_aoi"]:
        print("WARNING: the DEM on disk does NOT fully cover the canonical Assam AOI. "
    ...
    if verifications.get("has_boundary_artifacts"):
        raise RuntimeError(
            "Chunk-boundary artifacts detected in the Assam terrain rasters "
