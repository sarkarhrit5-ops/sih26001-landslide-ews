#!/usr/bin/env python3
"""
Train the Arunachal Pradesh pilot landslide model from the completed 200x11
training matrix.

This is the Arunachal analogue of ``scripts/train_assam_model.py`` (itself the
Assam analogue of ``scripts/train_real_models.py`` sections 4-7:
validate -> decide -> persist). It deliberately REUSES the Sikkim/Assam
...
Sikkim:
    ...
    * ``land_cover_class`` is fed to the LightGBM PRIMARY model as a pandas
      ...
      the Arunachal feature -- exactly like Assam's -- is a REAL, NOMINAL ESA
      WorldCover class grouping (forest / shrub / cropland / built-up / bare /
      water) that has no natural order, unlike Sikkim's elevation-binned proxy,
      which is an ordered 1 < 2 < 3 encoding and is therefore left numeric there.

Everything else is identical to Sikkim/Assam: the 11-feature order, the
...
Hard integrity rules (shared with the whole project):
    ...
    * It does NOT rebuild features and it does NOT touch the network: it loads
      the matrix that ``scripts/build_arunachal_training_matrix.py`` already
      wrote.
    * It does NOT modify Sikkim, Assam, IMERG (``weather_ingestion.py``), the
      frontend, or any shared service. State/pilot labels that ``model_artifacts``
      hardcodes to "Sikkim" are corrected on the returned dicts (relabelling only
      -- no metric, feature or provenance value is altered).
...
# ---------------------------------------------------------------------------
# Arunachal Pradesh pilot constants
# ---------------------------------------------------------------------------
STATE_NAME = "Arunachal Pradesh"
PILOT_AREA = "central Subansiri-Siang belt"
MATRIX_FILENAME = "arunachal_pilot_training_matrix.parquet"
MATRIX_SCHEMA_FILENAME = "arunachal_pilot_training_matrix_schema.json"
...
# Same cutoff Sikkim/Assam use. Verified non-degenerate for Arunachal
# (positives: 20 train (<=2014) / 30 test (>=2015); distinct event dates
# 19 vs 19).
...
# ---------------------------------------------------------------------------
def arunachal_land_cover_meaning() -> str:
    """Human-readable meaning string for the Arunachal land_cover_class feature.
...
def arunachal_feature_meanings() -> dict:
    ...
    """
    return {wc.LANDCOVER_FEATURE_NAME: arunachal_land_cover_meaning()}


def relabel_for_arunachal(doc):
    ...
    they predate the Arunachal pilot and take no state argument. This rewrites
    ONLY those label fields to the real Arunachal run. It never touches
...
def train_primary_categorical(ml, X_train, X_test, y_train, y_test, categories):
    """Arunachal analogue of ml_pipeline.train_primary_model.
...
def evaluate_baselines_arunachal(ml, X_train, X_test, y_train, y_test, categories):
    """Arunachal analogue of ml_pipeline.train_and_evaluate_baselines.
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            "Train the Arunachal Pradesh pilot landslide model from the "
            "completed arunachal_pilot_training_matrix.parquet, mirroring the "
            "Sikkim/Assam methodology (WorldCover categorical, real rainfall "
            "only, leakage controls). Does not modify Sikkim, Assam or IMERG."
        )
    )
    parser.add_argument(
        "--matrix",
        default=None,
        help="Path to arunachal_pilot_training_matrix.parquet (default: backend/data/processed/).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    print("==========================================================")
    print("  ARUNACHAL PILOT MODEL TRAINING (Sikkim/Assam methodology) ")
    print("==========================================================")
    ...
    if not os.path.exists(matrix_path):
        raise SystemExit(
            "Training matrix not found: %s\nRun scripts/build_arunachal_training_matrix.py "
    ...
    # --- 5. Leakage controls (same wording as Sikkim, Arunachal counts) ---
    leakage_checks = {
        "spatial_leakage": (
    ...
    try:
        ...
        metrics_doc = relabel_for_arunachal(
        ...
        feature_schema_doc = relabel_for_arunachal(
            model_artifacts.build_feature_schema_document(
                feature_names=feature_columns,
                dtypes=fit_view_dtypes,
                meanings=arunachal_feature_meanings(),
        ...
        provenance_doc = model_artifacts.build_provenance_document(
            ...
            random_seed=42,
            glc_source=(
                "NASA Global Landslide Catalog Export (arunachal_pradesh_events.json "
            ...
            dem_source=(
                "Copernicus GLO-30 DEM (30 m), Arunachal pilot tiles, merged and "
            ...
            extra={
                "land_cover_treatment": (
                    ...
                    "It matches the Assam pilot's treatment exactly."
                    % (
                        wc.WORLDCOVER_VERSION,
                        wc.WORLDCOVER_YEAR,
                        wc.landcover_categorical_feature(),
                ...
                "imerg_note": (
                    ...
                    "archive, exactly as in Sikkim/Assam training."
                ),
                "matrix_schema_reference": schema_path,
                "aoi_source": (
                    "app.core.config_states.ARUNACHAL_PILOT_AOI -- the single "
                    "canonical Arunachal pilot AOI used for tile selection, DEM crop, "
