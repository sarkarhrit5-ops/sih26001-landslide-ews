"""
Build the ARUNACHAL PRADESH pilot TRAINING MATRIX (features only -- this script
NEVER trains a model). It is the Arunachal analogue of scripts/build_assam_training_matrix.py
(which is itself the Assam analogue of the feature-building portion of the Sikkim
trainer scripts/train_real_models.py), with exactly the SAME deliberate substitution
the Assam pilot made: the degenerate elevation land_cover_class proxy is replaced by
the REAL, categorical ESA WorldCover land cover produced by
scripts/prepare_arunachal_landcover.py.

FEATURE SCHEMA (11 model features, identical layout to the Sikkim/Assam trainers):
    5 terrain (float32) : elevation, slope, aspect, roughness, tpi
        -> sampled from the on-disk Arunachal rasters (arunachal_pilot_dem.tif +
           arunachal_pilot_{slope,aspect,roughness,tpi}.tif) with the SAME
    ...
    1 land cover (int32, CATEGORICAL) : land_cover_class
        -> app.services.worldcover.assign_assam_land_cover_from_raster (grouped,
           ...
           Arunachal raster, exactly as the grouping constants are. Declared
    ...
    5 rainfall (float32) : rain_1d, rain_3d, rain_7d, antecedent_rain_14d,
        ...
        -> the SAME scripts/train_real_models.fetch_historical_rainfall_series
           ...
           This is the identical rainfall source Sikkim/Assam TRAINING already use;
...
SAMPLES: all real GLC positives from data/models/arunachal_pradesh_events.json
(50 events, target=1) plus spatially-buffered negatives from
app.models.ml_pipeline.generate_spatial_negative_samples (target=0, 3:1 -> 150,
>= 0.05 deg from any positive, seed 42), drawn from exactly the canonical Arunachal
AOI rectangle (== the DEM/land-cover extent) -> 200 rows total. No sample is dropped
...
DATA-INTEGRITY CONTRACT (nothing is fabricated or filled):
    ...
    * Rainfall: on ANY failure to obtain a complete real antecedent series for ANY
      ...
      is written (mirrors the Sikkim/Assam trainers). Missing rainfall never becomes
      0 mm.
...
WHY THE arunachal_pilot_ NAMES: same collision rationale as the DEM/terrain/landcover
drivers -- the sweep (app.services.state_validation) owns the "arunachal_pradesh_"
prefix, so all pilot products use "arunachal_pilot_" to stay separate. The events
snapshot it consumes is the canonical UNDERSCORE arunachal_pradesh_events.json.
...
inside main(). It does NOT train, does NOT commit, and touches neither the Sikkim nor
Assam pipeline, IMERG, nor the frontend.

Usage (from backend/ on the host):
    python scripts/build_arunachal_training_matrix.py
    python scripts/build_arunachal_training_matrix.py --max-workers 8
"""
import argparse
...
EVENTS_FILENAME = "arunachal_pradesh_events.json"
DEM_FILENAME = "arunachal_pilot_dem.tif"
LANDCOVER_FILENAME = "arunachal_pilot_landcover.tif"
...
MATRIX_FILENAME = "arunachal_pilot_training_matrix.parquet"
SCHEMA_FILENAME = "arunachal_pilot_training_matrix_schema.json"

# Feature layout -- identical order/semantics to the Sikkim/Assam trainers'
# static_features + rainfall block, so an Arunachal trainer can reuse the same code.
...
# archive-api.open-meteo.com request per sample. Fetching all 200 samples with high
...
def load_positive_events(models_dir):
    """
    Load the real Arunachal GLC positives from the committed events snapshot into a
    ...
    if not os.path.exists(path):
        raise SystemExit("Arunachal events snapshot not found at %s." % path)
    with open(path, "r", encoding="utf-8") as fh:
        doc = json.load(fh)
    events = doc.get("events", [])
    if not events:
        raise SystemExit("Arunachal events snapshot at %s contains zero events." % path)
    ...
    if n_bad_date:
        ...
        raise SystemExit(
            "%d Arunachal positive event(s) have an unparseable event_date; refusing to "
...
def terrain_raster_map(raw_dir, proc_dir):
    ...
    if missing:
        raise SystemExit(
            "Missing terrain raster(s) for %s: %s. Run scripts/prepare_arunachal_terrain.py first."
...
def main():
    parser = argparse.ArgumentParser(
        description="Build the Arunachal pilot training feature matrix (no model training)."
    )
    parser.add_argument(
    ...
    # 0. Canonical AOI (fail fast if the pilot AOI is not inside Arunachal's admin bbox).
    ...
    if not os.path.exists(landcover_path):
        raise SystemExit(
            "WorldCover raster not found at %s. Run scripts/prepare_arunachal_landcover.py first."
            % landcover_path
        )
    raster_map = terrain_raster_map(raw_dir, proc_dir)

    print("=" * 72)
    print(" ARUNACHAL PILOT TRAINING MATRIX (features only; NO training)")
    ...
    #    it samples the given Arunachal raster with the shared nominal grouping.
    ...
    try:
        ...
        # Mirror the Sikkim/Assam trainers: do NOT fabricate rainfall and do NOT persist
        # a matrix built on incomplete rainfall. No artifact is written.
        raise trm.RainfallUnavailableError(
            "ABORTING: real Open-Meteo ERA5 antecedent rainfall is unavailable for one "
            "or more Arunachal samples, so a scientifically valid training matrix cannot "
    ...
    document = {
        ...
        "negative_sampling": {
            ...
            "seed": 42,
            "domain": "canonical Arunachal AOI rectangle (== DEM/land-cover extent)",
        },
        "integrity": {
