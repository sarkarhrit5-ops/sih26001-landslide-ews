...
canonical Arunachal Pradesh pilot AOI. This is the Arunachal analogue of the
DEM/terrain stage of scripts/train_real_models.py (and of the Assam driver
scripts/prepare_assam_terrain.py), extracted into its own driver so the Arunachal
data can be prepared WITHOUT touching the Sikkim or Assam pilots, the
state-comparison sweep, IMERG authentication or the frontend.

WHAT IT PRODUCES (all under backend/data/, none committed):
    data/raw/arunachal_pilot_dem.tif             -- 6-tile mosaic, cropped to the AOI
    data/processed/arunachal_pilot_slope.tif
    data/processed/arunachal_pilot_aspect.tif
    data/processed/arunachal_pilot_roughness.tif
    data/processed/arunachal_pilot_tpi.tif

WHY THESE NAMES (do not "simplify" to arunachal_dem.tif / arunachal_pradesh_dem.tif):
    ...
    "arunachal_pradesh_dem.tif" and process_dem_in_chunks(state_prefix=
    "arunachal_pradesh") -> "arunachal_pradesh_*.tif". If this pilot driver reused
    those bare names, acquire_state_dem's size-guard would then serve this
    pilot-AOI DEM in place of the sweep's admin-bbox DEM, silently changing the
    8-state sweep. The "arunachal_pilot_" prefix keeps the pilot cleanly separate
    -- exactly as "east_sikkim_dem" / "real_" do for Sikkim and "assam_pilot_" does
    for Assam -- and clashes with neither the sweep nor the other pilots. (It also
    differs from the events snapshot arunachal_pradesh_events.json only by design;
    that file is JSON, these are rasters.)

WHY 6 TILES, NOT 9: the canonical AOI's max_lat is 27.99 (not 28.0) precisely so
    get_dem_tiles_for_bbox -- which floors min/max lat & lon -- resolves to the six
    tiles N26/N27 x E092/E093/E094. An integer 28.0 would floor to 28 and pull in a
    third tile row (N28*, ~28-29 N) that lies OUTSIDE the AOI, inflating the mosaic
    to nine tiles for no coverage gain. See the comment on ARUNACHAL_PILOT_AOI in
    app/core/config_states.py.

REUSE, NOT REINVENTION: every AOI-dependent step reads the AOI from
app.core.config_states.get_pilot_aoi_bounds("Arunachal Pradesh") (the single source
of truth), selects Copernicus tiles with the same app.services.state_validation
...
ALL tiles the AOI needs (6), exactly like the uncapped pilot path.

RAM NOTE (8 GB target): the Arunachal AOI raster is ~1.56x the Assam pilot's, which
is the main memory watch-item on the 8 GB host. The DEM mosaic and terrain pass are
both streamed/chunked (chunk_size default 512), and _print_ram() reports RSS at each
stage so a run that approaches the limit is visible rather than silent.
...
Usage (from the backend/ directory, on the host):
    python scripts/prepare_arunachal_terrain.py            # build if missing
    python scripts/prepare_arunachal_terrain.py --force    # rebuild even if present
"""
import argparse
import os
import sys
...
# Pilot-specific output names (see module docstring for why they are NOT "arunachal_"
# or "arunachal_pradesh_").
DEM_FILENAME = "arunachal_pilot_dem.tif"
TERRAIN_STATE_PREFIX = "arunachal_pilot"  # process_dem_in_chunks appends "_" -> arunachal_pilot_*.tif
...
def build_arunachal_dem(raw_dir, tile_cache_dir, force=False):
    """
    Download the Copernicus GLO-30 tiles the Arunachal pilot AOI needs (UNCAPPED)
    and mosaic+crop them to exactly the AOI, writing data/raw/arunachal_pilot_dem.tif.
    ...
    print("Copernicus GLO-30 tiles required by the canonical Arunachal AOI: %s" % tiles)
    ...
    if not tile_paths:
        raise RuntimeError("No Copernicus tiles resolved for the Arunachal pilot AOI.")
    ...
    print("Wrote Arunachal pilot DEM: %s" % dem_path)
    return dem_path, aoi, tiles
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare the real Copernicus DEM + terrain derivatives for the Arunachal pilot AOI."
    )
    parser.add_argument(
        "--force", action="store_true",
        help="Rebuild the DEM mosaic even if data/raw/arunachal_pilot_dem.tif already exists.",
    )
    parser.add_argument(
    ...
    # Fail fast if the pilot AOI is not inside Arunachal's administrative box; in
    ...
    print("=" * 70)
    print(" ARUNACHAL PRADESH PILOT DEM + TERRAIN PREPARATION")
    ...
    dem_path, aoi, tiles = build_arunachal_dem(raw_dir, tile_cache_dir, force=args.force)
    ...
    if not coverage["fully_covers_canonical_aoi"]:
        print("WARNING: the DEM on disk does NOT fully cover the canonical Arunachal AOI. "
    ...
    if verifications.get("has_boundary_artifacts"):
        raise RuntimeError(
            "Chunk-boundary artifacts detected in the Arunachal terrain rasters "
