"""
ARUNACHAL PRADESH GRID PREDICTION SERVICE

Runs the persisted rainfall-coupled Arunachal Pradesh LightGBM over a coarse grid
tiling the canonical Arunachal pilot AOI (central Subansiri-Siang belt) and returns,
per grid cell, the model's raw susceptibility probability and the system warning
class. It is the Arunachal analogue of app.services.assam_prediction and, exactly
like that module, deliberately REUSES app.services.sikkim_prediction's state-agnostic
building blocks (grid construction, the real IMERG antecedent-rainfall provider,
model-evidence resolution/validation, rainfall feature derivation, warning-band
mapping, report assembly and the UNAVAILABLE marker) so the three pilots cannot drift
apart. Sikkim, Assam, IMERG (weather_ingestion) and the trained models are NOT
modified by this module.

TWO -- AND ONLY TWO -- THINGS DIFFER FROM THE SIKKIM PATH (identical to Assam):
  ...
  1. LAND COVER IS REAL, NOT AN ELEVATION PROXY. Sikkim derives land_cover_class
     ...
     Arunachal instead samples REAL ESA WorldCover v200 (2021) at each cell via
     app.services.worldcover.sample_worldcover_at_points over the Arunachal land-cover
  ...
  2. LAND COVER IS SCORED AS A CATEGORICAL. The persisted Arunachal model was fit
     ...
     rainfall stay float32. This treatment matches the Assam pilot exactly.
...
sikkim_prediction.py / assam_prediction.py): this module answers only from real,
resolved inputs or it refuses. It never fabricates a coordinate, terrain value,
land-cover class, rainfall value or prediction, never back-fills a nodata cell with a
placeholder, and never hard-codes a probability. A cell whose terrain OR land cover
is missing / nodata / out-of-coverage is returned with status UNAVAILABLE and NO
probability; if the model artifacts or the real rainfall cannot be obtained, the whole
request is refused (PredictionUnavailable -> HTTP 503).
...
Arunachal rasters, fetch real IMERG, predict_proba) is host-only.
"""
...
# Re-using these (rather than re-declaring them) guarantees the Arunachal path stays
...
# --- Arunachal-specific identity + on-disk artifacts ---------------------------
STATE_NAME = "Arunachal Pradesh"
PILOT_AREA = "central Subansiri-Siang belt"

# Arunachal raster filenames -- byte-for-byte the ones
# scripts/build_arunachal_training_matrix.py consumed to build the matrix the model
# was fit on (DEM + land cover under data/raw, terrain derivatives under
# data/processed).
ARUNACHAL_DEM_FILENAME = "arunachal_pilot_dem.tif"
ARUNACHAL_LANDCOVER_FILENAME = "arunachal_pilot_landcover.tif"
ARUNACHAL_TERRAIN_DERIVATIVE_FILENAMES = {
    "slope": "arunachal_pilot_slope.tif",
    "aspect": "arunachal_pilot_aspect.tif",
    "roughness": "arunachal_pilot_roughness.tif",
    "tpi": "arunachal_pilot_tpi.tif",
}
...
# ---------------------------------------------------------------------------
# Arunachal raster path helpers (mirror risk_inputs.terrain_raster_paths, Arunachal names)
# ---------------------------------------------------------------------------
def arunachal_terrain_raster_paths(data_dir=None):
    """
    Absolute paths of the five real Arunachal terrain rasters, keyed by feature name.

    Uses the SAME data root as the rest of the backend (risk_inputs.default_data_dir
    -> backend/data): the DEM at data/raw/arunachal_pilot_dem.tif and the derivative
    rasters at data/processed/arunachal_pilot_<name>.tif.
    """
    root = os.path.abspath(data_dir or risk_inputs.default_data_dir())
    paths = {"elevation": os.path.join(root, "raw", ARUNACHAL_DEM_FILENAME)}
    for name, fname in ARUNACHAL_TERRAIN_DERIVATIVE_FILENAMES.items():
        paths[name] = os.path.join(root, "processed", fname)
    return paths


def arunachal_landcover_raster_path(data_dir=None):
    """Absolute path of the real Arunachal ESA WorldCover raster (data/raw)."""
    root = os.path.abspath(data_dir or risk_inputs.default_data_dir())
    return os.path.join(root, "raw", ARUNACHAL_LANDCOVER_FILENAME)


def missing_arunachal_terrain_rasters(data_dir=None):
    """[(feature_name, path)] for every Arunachal terrain raster that is absent or empty."""
    return [
        (name, path)
        for name, path in sorted(arunachal_terrain_raster_paths(data_dir).items())
        if not (os.path.exists(path) and os.path.getsize(path) > 0)
    ]


# ---------------------------------------------------------------------------
# Terrain (real Arunachal rasters; batched to open each raster once)
# ---------------------------------------------------------------------------
def _arunachal_terrain_sampler(centers, data_dir=None, rasterio_module=None):
    """
    Batched real-terrain sampler over the Arunachal rasters. Same rules as
    ...
    Raises PredictionUnavailable if any Arunachal terrain raster is missing/empty or
    ...
    refusal.
    """
    missing = missing_arunachal_terrain_rasters(data_dir)
    if missing:
        raise PredictionUnavailable(
            "Missing or empty Arunachal terrain raster(s): "
    ...
    if rasterio_module is None:
        try:
            ...
            raise PredictionUnavailable(
                "rasterio is not installed, so the real Arunachal terrain rasters "
                "cannot be read (%s)." % exc
            )

    paths = arunachal_terrain_raster_paths(data_dir)
    n = len(centers)
...
# ---------------------------------------------------------------------------
def _arunachal_land_cover(centers, data_dir=None, reader=None):
    """
    Real WorldCover land-cover group per cell, sampled from the Arunachal WorldCover
    ...
    as the Arunachal training pipeline drops such rows instead of imputing them.
    ...
    """
    path = arunachal_landcover_raster_path(data_dir)
    if not (os.path.exists(path) and os.path.getsize(path) > 0):
        raise PredictionUnavailable(
            "Missing or empty Arunachal WorldCover land-cover raster: %s" % path,
...
def _score_cells_categorical(model, feature_rows):
    ...
    the persisted Arunachal model was fit on (categorical_feature=['land_cover_class']).
    """
...
def _arunachal_disclosures():
    return [
        ...
        "TRAIN/SERVE RAINFALL-SOURCE SHIFT: the Arunachal Pradesh model was trained on "
        ...
        "This categorical, real land cover (matching the Assam pilot's treatment "
        "exactly) is the ONLY methodological difference from the Sikkim pilot, which "
        "uses an ordered elevation proxy left numeric."
...
# Orchestration
# ---------------------------------------------------------------------------
def predict_arunachal_grid(target_date, step_deg=DEFAULT_STEP_DEG, run_type="Early",
    ...
    """
    Full read-only prediction over the coarse Arunachal pilot-AOI grid. See the module
    docstring for the data-integrity contract and the two Arunachal-specific
    ...
    # 1. Model (real Arunachal artifacts, correct 11-feature order) -- refuse if unusable.
    evidence = sp._resolve_model_evidence(model_evidence, state_name, artifact_dir)
    model = evidence["model"]

    # 2. Grid over the canonical Arunachal AOI.
    ...
    # 4. Terrain per cell (real Arunachal rasters).
    sampler = terrain_sampler or _arunachal_terrain_sampler
    ...
    resolver = land_cover_resolver or _arunachal_land_cover
    ...
    return {
        ...
        "summary": summary,
        "disclosures": _arunachal_disclosures(),
        "cells": cells,
    }
