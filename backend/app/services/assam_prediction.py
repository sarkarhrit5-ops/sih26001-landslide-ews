"""
ASSAM GRID PREDICTION SERVICE

Runs the persisted rainfall-coupled Assam LightGBM over a coarse grid tiling the
canonical Assam pilot AOI (Guwahati-Kamrup + western Karbi Anglong) and returns,
per grid cell, the model's raw susceptibility probability and the system warning
class. It is the Assam analogue of app.services.sikkim_prediction and deliberately
REUSES that module's state-agnostic building blocks (grid construction, the real
IMERG antecedent-rainfall provider, model-evidence resolution/validation, rainfall
feature derivation, warning-band mapping, report assembly and the UNAVAILABLE
marker) so the two pilots cannot drift apart. Sikkim, IMERG (weather_ingestion) and
the trained models are NOT modified by this module.

TWO -- AND ONLY TWO -- THINGS DIFFER FROM THE SIKKIM PATH:

  1. LAND COVER IS REAL, NOT AN ELEVATION PROXY. Sikkim derives land_cover_class
     from an elevation binning (risk_inputs.land_cover_class_from_elevation). Assam
     instead samples REAL ESA WorldCover v200 (2021) at each cell via
     app.services.worldcover.sample_worldcover_at_points over the Assam land-cover
  ...
  2. LAND COVER IS SCORED AS A CATEGORICAL. The persisted Assam model was fit with
     land_cover_class as a pandas categorical over the FIXED WorldCover vocabulary
     list(worldcover.ASSAM_LANDCOVER_GROUP_CODES) == (1,2,3,4,5,6) and
...
sikkim_prediction.py): this module answers only from real, resolved inputs or it
refuses. It never fabricates a coordinate, terrain value, land-cover class, rainfall
value or prediction, never back-fills a nodata cell with a placeholder, and never
hard-codes a probability. A cell whose terrain OR land cover is missing / nodata /
out-of-coverage is returned with status UNAVAILABLE and NO probability; if the model
artifacts or the real rainfall cannot be obtained, the whole request is refused
(PredictionUnavailable -> HTTP 503).
...
Assam rasters, fetch real IMERG, predict_proba) is host-only.
"""
...
# Re-using these (rather than re-declaring them) guarantees the Assam path stays in
# lock-step with the Sikkim path for everything that is genuinely common.
...
# --- Assam-specific identity + on-disk artifacts -------------------------------
STATE_NAME = "Assam"
PILOT_AREA = "Guwahati-Kamrup + western Karbi Anglong"

# Assam raster filenames -- byte-for-byte the ones scripts/build_assam_training_matrix.py
# consumed to build the matrix the model was fit on (DEM + land cover under data/raw,
# terrain derivatives under data/processed).
ASSAM_DEM_FILENAME = "assam_pilot_dem.tif"
ASSAM_LANDCOVER_FILENAME = "assam_pilot_landcover.tif"
...
# training, so the model reads the same integer category codes it learned on.
LAND_COVER_CATEGORIES = list(wc.ASSAM_LANDCOVER_GROUP_CODES)  # (1,2,3,4,5,6)


# ---------------------------------------------------------------------------
# Assam raster path helpers (mirror risk_inputs.terrain_raster_paths, Assam names)
# ---------------------------------------------------------------------------
def assam_terrain_raster_paths(data_dir=None):
    """
    Absolute paths of the five real Assam terrain rasters, keyed by feature name.

    Uses the SAME data root as the rest of the backend (risk_inputs.default_data_dir
    -> backend/data): the DEM at data/raw/assam_pilot_dem.tif and the derivative
    rasters at data/processed/assam_pilot_<name>.tif.
    """
    root = os.path.abspath(data_dir or risk_inputs.default_data_dir())
    paths = {"elevation": os.path.join(root, "raw", ASSAM_DEM_FILENAME)}
    for name, fname in ASSAM_TERRAIN_DERIVATIVE_FILENAMES.items():
        paths[name] = os.path.join(root, "processed", fname)
    return paths


def assam_landcover_raster_path(data_dir=None):
    """Absolute path of the real Assam ESA WorldCover raster (data/raw)."""
    root = os.path.abspath(data_dir or risk_inputs.default_data_dir())
    return os.path.join(root, "raw", ASSAM_LANDCOVER_FILENAME)


def missing_assam_terrain_rasters(data_dir=None):
    """[(feature_name, path)] for every Assam terrain raster that is absent or empty."""
    return [
        (name, path)
        for name, path in sorted(assam_terrain_raster_paths(data_dir).items())
        if not (os.path.exists(path) and os.path.getsize(path) > 0)
    ]


# ---------------------------------------------------------------------------
# Terrain (real Assam rasters; batched to open each raster once)
# ---------------------------------------------------------------------------
def _assam_terrain_sampler(centers, data_dir=None, rasterio_module=None):
    """
    Batched real-terrain sampler over the Assam rasters. Same rules as
    ...
    Raises PredictionUnavailable if any Assam terrain raster is missing/empty or
    ...
    refusal.
    """
    missing = missing_assam_terrain_rasters(data_dir)
    if missing:
        raise PredictionUnavailable(
            "Missing or empty Assam terrain raster(s): "
    ...
    if rasterio_module is None:
        try:
            ...
            raise PredictionUnavailable(
                "rasterio is not installed, so the real Assam terrain rasters cannot "
                "be read (%s)." % exc
            )

    paths = assam_terrain_raster_paths(data_dir)
    n = len(centers)
...
# ---------------------------------------------------------------------------
def _assam_land_cover(centers, data_dir=None, reader=None):
    """
    Real WorldCover land-cover group per cell, sampled from the Assam WorldCover
    ...
    as the Assam training pipeline drops such rows instead of imputing them.
    ...
    """
    path = assam_landcover_raster_path(data_dir)
    if not (os.path.exists(path) and os.path.getsize(path) > 0):
        raise PredictionUnavailable(
            "Missing or empty Assam WorldCover land-cover raster: %s" % path,
...
def _score_cells_categorical(model, feature_rows):
    ...
    the persisted Assam model was fit on (categorical_feature=['land_cover_class']).
    """
...
def _assam_disclosures():
    return [
        ...
        "TRAIN/SERVE RAINFALL-SOURCE SHIFT: the Assam model was trained on Open-Meteo "
        "ERA5 reanalysis rainfall (provenance records IMERG as NOT_USED at training); "
        "this endpoint serves NASA GPM IMERG. The sources differ, which may affect "
        ...
        "This categorical, real land cover is the ONLY methodological difference from "
        "the Sikkim pilot, which uses an ordered elevation proxy left numeric."
...
# Orchestration
# ---------------------------------------------------------------------------
def predict_assam_grid(target_date, step_deg=DEFAULT_STEP_DEG, run_type="Early",
    ...
    """
    Full read-only prediction over the coarse Assam pilot-AOI grid. See the module
    docstring for the data-integrity contract and the two Assam-specific differences
    (real WorldCover land cover + categorical scoring). The `model_evidence`,
    `terrain_sampler`, `land_cover_resolver` and `rainfall_provider` collaborators are
    injectable so the assembly and the no-fabrication invariants are testable offline
    with fakes; the defaults run the real host-only pipeline.
    ...
    # 1. Model (real Assam artifacts, correct 11-feature order) -- refuse if unusable.
    evidence = sp._resolve_model_evidence(model_evidence, state_name, artifact_dir)
    model = evidence["model"]

    # 2. Grid over the canonical Assam AOI.
    ...
    # 4. Terrain per cell (real Assam rasters).
    sampler = terrain_sampler or _assam_terrain_sampler
    ...
    resolver = land_cover_resolver or _assam_land_cover
    ...
    return {
        ...
        "generated_from": (
            ...
            "scored as a categorical) + real ESA WorldCover land cover + real IMERG "
            "antecedent rainfall"
        ),
        ...
        "summary": summary,
        "disclosures": _assam_disclosures(),
        "cells": cells,
    }
